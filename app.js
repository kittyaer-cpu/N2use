...
function render() {
  ...
  if (state.tab === 'history') {
    shell(`
      <div class="card">
        <h2>📜 獎懲紀錄</h2>
        <button id="exportImg" class="gold">匯出罰款結算圖 PNG</button>
      ...
      <div id="receipt" class="receipt">
        ...
        ${state.records.length ? state.records.map(r => `
          <p>${r.type === 'fine' ? '罰款' : '獎勵'}｜${r.store || '整區'}｜${r.reason}｜$${r.amount}｜${r.createdByName}</p>
        `).join('') : '<p>目前沒有獎懲紀錄</p>'}
      </div>
...
function bindHistory() {
  ...
  $('#exportImg').onclick = async () => {
    ...
    try {
      ...
      link.download = `北二獎懲基金結算圖_${new Date().toLocaleDateString('zh-TW').replaceAll('/', '-')}.png`;
