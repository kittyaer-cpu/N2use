...
function storeFineTotals() {
  return stores.map(store => {
    const storeRecords = state.records.filter(r => r.type === 'fine' && r.store === store);
    const total = storeRecords.reduce((sum, r) => sum + (+r.amount || 0), 0);
    return {
      store,
      total,
...
function render() {
  ...
  if (state.tab === 'home') {
    return shell(`
      <div class="grid">
        <div class="card"><div>本月罰款</div><div class="stat">$${t.fine}</div></div>
        <div class="card"><div>本月獎勵</div><div class="stat">$${t.reward}</div></div>
        <div class="card"><div>基金支出</div><div class="stat">$${t.expense}</div></div>
        <div class="card"><div>預估區基金</div><div class="stat">$${t.balance}</div></div>
      </div>

      <div class="card">
        <h3>🌱 今日小提醒</h3>
        <p>罰款紀錄分店，獎勵紀錄整區；手動月結後，本月獎懲歸零並加總到區基金。</p>
      </div>
    `);
  }
  ...
  if (state.tab === 'fund') {
    shell(`
      <div class="card">
        <h2>💰 區基金帳本</h2>
        <div class="stat">目前預估 $${t.balance}</div>
  ...
  if (state.tab === 'history') {
    ...
    shell(`
      ...
      <div id="receipt" class="receipt">
        ...
        <div class="kpi">
          <div><b>罰款</b><br>$${t.fine}</div>
          <div><b>獎勵</b><br>$${t.reward}</div>
          <div><b>基金</b><br>$${t.balance}</div>
        </div>
        ...
        ${storeTotals.map(s => `
          <div class="row receipt-row">
            <div>
              <b>${s.store}</b>
              <div class="small muted">${s.count} 筆罰款紀錄</div>
            </div>
            <div><b>$${s.total}</b></div>
          </div>
        `).join('')}
        ...
        <h3>📌 罰款明細</h3>
        ${state.records.filter(r => r.type === 'fine').length ? state.records.filter(r => r.type === 'fine').map(r => `
          <p>罰款｜${r.store || '未指定'}｜${r.reason}｜$${r.amount}｜${r.createdByName}</p>
        `).join('') : '<p>目前沒有罰款紀錄</p>'}

        <hr>

        <h3>⭐ 整區獎勵明細</h3>
        ${state.records.filter(r => r.type === 'reward').length ? state.records.filter(r => r.type === 'reward').map(r => `
          <p>獎勵｜整區｜${r.reason}｜$${r.amount}｜${r.createdByName}</p>
        `).join('') : '<p>目前沒有獎勵紀錄</p>'}
      </div>
